...
def create_output_directory(script_name):
    """
    Create a directory for saving output results. The directory name consists of the script name and the current timestamp.
    """
...
def train_and_evaluate(data, feature_name, time_step, company_name, hidden_layer_size, num_layers, lr, version, output_dir):
    ...
    train_loader = DataLoader(train_dataset, batch_size=8, shuffle=True)  # Batch size reduced to 8
    ...
    for epoch in range(epochs):
        ...
        for i, (X_batch, Y_batch) in enumerate(train_loader):
            ...
            with torch.cuda.amp.autocast():  # Use autocast for mixed precision
                ...
                loss = loss_function(y_pred, Y_batch) / accumulation_steps  # Divide by accumulation steps per update

            scaler.scale(loss).backward()

            # Perform optimizer update only after accumulation steps
    ...
    # Evaluate prediction error on training set using data loader
    train_predict = []
    with torch.no_grad():
        for X_batch, _ in train_loader:
            y_pred = model(X_batch)
            train_predict.extend(y_pred.cpu().numpy())

    train_predict = np.array(train_predict).flatten()  # Convert to 1D array

    # Calculate evaluation metrics
    train_rmse = np.sqrt(mean_squared_error(Y_train.cpu().numpy().flatten(), train_predict))
...
def main():
    ...
    for feature_name, scaled_data in [('Close Price', scaled_close_price), ('Volume', scaled_volume)]:
        train_losses, val_losses, train_rmse, test_rmse, test_mae, test_r2 = train_and_evaluate(
            scaled_data, feature_name, time_step=156, company_name=company, hidden_layer_size=64, num_layers=2, lr=0.001, version=version, output_dir=output_dir
        )
        results.append({
            'version': version,
...
torch.cuda.set_per_process_memory_fraction(0.5, device=0)  # Use 50% of GPU memory
