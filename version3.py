...
def main():
    ...
    for feature_name, scaled_data in [('Close Price', scaled_close_price), ('Volume', scaled_volume)]:
        ...
        for hidden_layer_size, num_layers, lr, time_step in product(hidden_layer_sizes, num_layers_list, learning_rates, time_steps):
            try:
                train_losses, val_losses, train_rmse, test_rmse, test_mae, test_r2 = train_and_evaluate(
                    scaled_data, feature_name, time_step=time_step, company_name=company, hidden_layer_size=hidden_layer_size, num_layers=num_layers, lr=lr, version=version, output_dir=output_dir
                )
            except RuntimeError as e:
                if 'out of memory' in str(e):
                    logger.warning(f"Skipping configuration due to memory error: hidden_layer_size={hidden_layer_size}, num_layers={num_layers}, lr={lr}, time_step={time_step}")
                    torch.cuda.empty_cache()
                    continue
                else:
                    raise e
